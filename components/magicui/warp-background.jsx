...
export const WarpBackground = ({
  ...
  // Add state to track if we're rendering on the client
  const [isMounted, setIsMounted] = useState(false);

  // Set mounted state once component mounts in the browser
  useEffect(() => {
    setIsMounted(true);
  }, []);

  const MotionComponent = motion.create('div');
  
  const generateBeams = useCallback(() => {
    // Don't generate beams during SSR
    if (!isMounted) return [];
    ...
    return beams;
  }, [beamsPerSide, beamSize, beamDelayMax, beamDelayMin, isMounted]);
